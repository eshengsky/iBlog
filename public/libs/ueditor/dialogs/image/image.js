    ...
    RemoteImage.prototype = {
        ...
        setImage: function (img) {
            ...
            if (src) {
                ...
                $G("border").value = UE.dom.domUtils.getStyle(img, "border");
                $G("vhSpace").value = img.getAttribute("vspace") || '0';
                $G("title").value = img.title || img.alt || '';
                var className = img.getAttribute("class") || '';
                $G("className").value = className;
                var radios = document.getElementsByName("styleType");
                for (var i = 0; i < radios.length; i++) {
                    if (radios[i].value == className) {
                ...
                return [
                    {
                        ...
                        style: "width:" + data['width'] + "px;height:" + data['height'] + "px;border:" + border + ";"
                    }];
            } else {
                return [];
            }
        ...
        initUploader: function () {
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        /* 添加额外的GET参数 */
                        var params = utils.serializeParam(editor.queryCommandValue('serverparam')) || 'uniqueId=' + parent.document.getElementById('UniqueId').value,
